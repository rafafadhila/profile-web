        ...
        >
            <div className="container max-w-4xl mx-auto px-5 py-10 text-center">
                ...
                <ScrollReveal direction='up' delay={0.4}>
                    <motion.div layout className="grid grid-cols-1 md:grid-cols-2 gap-5 tracking-wide">
                        ...
                        <AnimatePresence>
                            {displayedProjects.map((project) => (
                                <motion.div
                                    ...
                                    key={project.id}
                                    className={`flex flex-col rounded-xl overflow-hidden shadow-md ${isDark ? 'bg-neutral-800 text-white' : 'bg-white text-black'
                                        } transition-colors`}
                                >
                                    <div className="w-full h-64 p-3 shrink-0">
                                        <div className="relative w-full h-full rounded-md overflow-hidden">
                                            <Image
                                                ...
                                                priority={reversedData.findIndex(p => p.id === project.id) < 2}
                                            />
                                        </div>
                                    </div>

                                    {/* Tambahkan flex-grow agar teks memenuhi ruang yang tersisa jika deskripsi pendek */}
                                    <div className="px-5 py-4 text-left flex flex-col flex-grow">

                                        {/* 2. Flexbox untuk Judul dan Ikon Link */}
                                        <div className="flex justify-between items-start mb-2 gap-3">
                                            <h2 className="font-semibold text-xl leading-tight">{project.title}</h2>

                                            {/* Wadah Ikon */}
                                            <div className="flex gap-2 mt-1 shrink-0">
                                                {/* Cek apakah repo ada isinya */}
                                                {project.repo && (
                                                    <a
                                                        href={project.repo}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        // Tambahkan class untuk background, border, padding, dan hover effects
                                        ...
                                        <p className="mb-4 text-sm flex-grow">{project.description}</p>
